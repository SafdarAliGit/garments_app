// Copyright (c) 2024, Tech Ventured and contributors
// For license information, please see license.txt

// Copyright (c) 2021, Unilink Enterprise and contributors
// For license information, please see license.txt

frappe.ui.form.on('Cost Sheet Woven', {
    refresh(frm) {
        frm.add_custom_button(__('Sample Form'), function () {
            frappe.new_doc('Sample Form', {
                ...
                'costing_parenttype': 'Cost Sheet Woven',
...
frappe.ui.form.on('Fabric Calculations Woven', {
    ...
    product_size_width(frm, cdt, cdn) {
        length_mtr(frm, cdt, cdn);
        per_pcs_width(frm, cdt, cdn);
        per_pcs_length(frm, cdt, cdn);
        required_fabric(frm, cdt, cdn);
    ...
    product_size_length(frm, cdt, cdn) {
        length_mtr(frm, cdt, cdn);
        per_pcs_width(frm, cdt, cdn);
        per_pcs_length(frm, cdt, cdn);
        required_fabric(frm, cdt, cdn);
...
function length_mtr(frm, cdt, cdn) {
    var d = locals[cdt][cdn];
    var length_mtr = flt(d.length || 1) / 39.36;
    frappe.model.set_value(d.doctype, d.name, "length_mtr", length_mtr);
}

function per_pcs_width(frm, cdt, cdn) {
    var d = locals[cdt][cdn];
    var per_pcs_width = flt(d.required_width || 1) / flt(d.product_size_width || 1);
    frappe.model.set_value(d.doctype, d.name, "per_pcs_width", per_pcs_width);
}

function per_pcs_length(frm, cdt, cdn) {
    var d = locals[cdt][cdn];
    var per_pcs_length = flt(d.product_size_length) / 39.37;
    frappe.model.set_value(d.doctype, d.name, "per_pcs_length", per_pcs_length);
}
